...
@dataclass(frozen=True)
class MapPersistenceConfig:
    enabled: bool
    voxel_size_m: float
    min_observations: int
    decay_per_missed_keyframe: float
    max_decay_gap_keyframes: int
...
def _derive_map_point_chunks(
    ...
    point_chunks: list[tuple[int, np.ndarray]] = []
    for keyframe_index, keyframe in enumerate(keyframes):
        if keyframe.points_local.size == 0:
            continue
        ...
        if points_local.size == 0:
            continue
        point_chunks.append((keyframe_index, _transform_points(points_local, poses_xyyaw[keyframe_index])))
    return point_chunks


def _stack_map_point_chunks(point_chunks: list[tuple[int, np.ndarray]]) -> np.ndarray:
    world_points = [points_xy for _, points_xy in point_chunks if points_xy.size > 0]
    if not world_points:
        return np.empty((0, 2), dtype=np.float64)
    return np.vstack(world_points)
...
def _derive_map_points(
    ...
    return _stack_map_point_chunks(_derive_map_point_chunks(keyframes, poses_xyyaw, local_edges))


def _filter_map_point_chunks_by_persistence(
    point_chunks: list[tuple[int, np.ndarray]],
    config: MapPersistenceConfig,
) -> tuple[np.ndarray, dict[str, object]]:
    raw_points = _stack_map_point_chunks(point_chunks)
    raw_point_count = int(raw_points.shape[0])
    if (
        not config.enabled
        or config.min_observations <= 1
        or config.voxel_size_m <= 1.0e-6
        ...
        return raw_points, {
            ...
            "confidence_threshold": float(max(1.0, float(config.min_observations) - 0.5)),
            "decay_per_missed_keyframe": float(config.decay_per_missed_keyframe),
            "max_decay_gap_keyframes": int(config.max_decay_gap_keyframes),
    ...
    min_observations = max(1, int(config.min_observations))
    confidence_threshold = max(1.0, float(min_observations) - 0.5)
    decay_per_missed = max(0.0, float(config.decay_per_missed_keyframe))
    max_decay_gap = max(0, int(config.max_decay_gap_keyframes))
    voxel_size_m = float(config.voxel_size_m)

    states: dict[tuple[int, int], dict[str, object]] = {}
    points_by_voxel: dict[tuple[int, int], list[np.ndarray]] = {}

    for keyframe_index, points_xy in point_chunks:
        if points_xy.size == 0:
            continue
        ...
        for unique_index, voxel in enumerate(unique_voxels):
            ...
            if state is None:
                ...
                observations = 0
                max_confidence = 0.0
                last_keyframe_index = int(keyframe_index)
            else:
                confidence = float(state["confidence"])
                observations = int(state["observations"])
                max_confidence = float(state["max_confidence"])
                last_keyframe_index = int(state["last_keyframe_index"])
                missed_keyframes = max(0, int(keyframe_index) - last_keyframe_index - 1)
                if missed_keyframes > 0 and decay_per_missed > 0.0:
                    decay_gap = missed_keyframes if max_decay_gap <= 0 else min(missed_keyframes, max_decay_gap)
    ...
    kept_points_parts: list[np.ndarray] = []
    for voxel_key in sorted(kept_voxels):
        voxel_chunks = points_by_voxel.get(voxel_key, [])
        if voxel_chunks:
            kept_points_parts.append(np.vstack(voxel_chunks))

    kept_points = (
        np.vstack(kept_points_parts)
        if kept_points_parts
    ...
    return kept_points, {
        ...
        "decay_per_missed_keyframe": decay_per_missed,
        "max_decay_gap_keyframes": max_decay_gap,
        "raw_point_count": raw_point_count,
...
def _run_mapping(args: argparse.Namespace) -> dict[str, object]:
    ...
    persistence_config = MapPersistenceConfig(
        enabled=bool(args.map_persistence_filter),
        voxel_size_m=(
            float(args.map_persistence_voxel_size_m)
            if float(args.map_persistence_voxel_size_m) > 1.0e-6
            else max(float(args.map_resolution_m), 2.0 * float(args.map_resolution_m))
        ),
        min_observations=max(1, int(args.map_persistence_min_observations)),
        decay_per_missed_keyframe=max(0.0, float(args.map_persistence_decay_per_missed_keyframe)),
        max_decay_gap_keyframes=max(0, int(args.map_persistence_max_decay_gap_keyframes)),
    )
    print(
    ...
    if persistence_config.enabled:
        print(
            "[general_mapper] filtro de persistencia mapa: voxel=%.3fm min_obs=%d decay=%.2f gap=%d"
            % (
                persistence_config.voxel_size_m,
                persistence_config.min_observations,
                persistence_config.decay_per_missed_keyframe,
                persistence_config.max_decay_gap_keyframes,
            ),
            flush=True,
        )
    ...
    candidate_persistence: dict[str, dict[str, object]] = {}
    for candidate_name, candidate_poses in candidate_poses_xyyaw.items():
        map_point_chunks = _derive_map_point_chunks(keyframes, candidate_poses, local_edges)
        map_points_xy, persistence_stats = _filter_map_point_chunks_by_persistence(
            map_point_chunks,
            persistence_config,
        )
        candidate_persistence[candidate_name] = persistence_stats
    ...
    summary_payload = {
        ...
        "candidate_map_persistence": candidate_persistence,
    ...
    if persistence_config.enabled:
        selected_persistence = candidate_persistence.get(selected_candidate_name, {})
        print(
            "[general_mapper] persistencia mapa: puntos %s/%s voxels %s/%s"
            % (
                selected_persistence.get("kept_point_count", 0),
                selected_persistence.get("raw_point_count", 0),
                selected_persistence.get("kept_voxel_count", 0),
                selected_persistence.get("raw_voxel_count", 0),
            ),
            flush=True,
        )
...
def main() -> None:
    ...
    parser.add_argument("--map-persistence-filter", dest="map_persistence_filter", action="store_true", default=True)
    parser.add_argument("--no-map-persistence-filter", dest="map_persistence_filter", action="store_false")
    parser.add_argument("--map-persistence-voxel-size-m", type=float, default=0.08)
    parser.add_argument("--map-persistence-min-observations", type=int, default=3)
    parser.add_argument("--map-persistence-decay-per-missed-keyframe", type=float, default=0.35)
    parser.add_argument("--map-persistence-max-decay-gap-keyframes", type=int, default=8)
