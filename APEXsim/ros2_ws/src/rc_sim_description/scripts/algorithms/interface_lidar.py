...
try:
    from algorithm_visualizer import *  # noqa: F403
    _HAS_VIS = True
except Exception:
    _HAS_VIS = False

    class VoitureAlgorithmPlotter:
        def updateView(self, *_args, **_kwargs):
            pass
...
try:
    ...
    try:
        ...
        class _DummyLogger:
            ...
            def logConsole(self, *_args, **_kwargs):
                pass
...
def _run_gazebo_lidar_process(
    ...
    scan_pub = None
    if publish_topic:
        scan_pub = node.create_publisher(LaserScan, publish_topic, 10)
    ...
    def scan_cb(msg: LaserScan):
        ...
        # Fill zeros by propagating last known reading (same behavior as real reader)
        ...
        if scan_pub is not None:
            ...
            out.header.frame_id = publish_frame_id or msg.header.frame_id
            out.angle_min = 0.0
            out.angle_max = 2.0 * math.pi
            out.angle_increment = (2.0 * math.pi) / 360.0
            out.time_increment = msg.time_increment
            out.scan_time = msg.scan_time
            out.range_min = msg.range_min
            out.range_max = msg.range_max
            out.ranges = shifted_distances.tolist()
            out.intensities = []
...
class RPLidarReader(LiDarInterface):
    ...
    def __init__(
        ...
        if not _HAS_RPLIDAR:
            raise RuntimeError("rplidar not available. Install rplidar or use GazeboLidarReader.")
...
class GazeboLidarReader(LiDarInterface):
    last_lidar_read = mp.Array('d', 360)
    last_lidar_update = mp.Value('d', 0.0)
    stop_event = mp.Event()
    ...
    def __init__(
        ...
        fov_filter: int = LIDAR_FOV_FILTER,
        point_timeout_ms: int = LIDAR_POINT_TIMEOUT_MS,
        publish_topic: str = "/lidar_processed",
        publish_frame_id: str = "",
        use_sim_time: bool = True,
        sensor_name: str = "Lidar",
        node_name: str = "gazebo_lidar_reader",
        log_scans: bool = False,
    ):
        if not _HAS_ROS:
            raise RuntimeError("ROS2 not available (rclpy import failed).")
        ...
        self.publish_frame_id = publish_frame_id
    ...
    def _start_lidar_process(self):
        if self._lidar_process is not None and self._lidar_process.is_alive():
            self.sensor_logger.info("[GazeboLidarReader] Process already running.")
            return

        self._lidar_process = mp.Process(
            target=_run_gazebo_lidar_process,
            args=(
                self.topic,
                self.node_name,
                self.use_sim_time,
        ...
        self._lidar_process.start()
        self.sensor_logger.info("[GazeboLidarReader] Process started.")
    ...
    def stop(self):
        self.sensor_logger_instance.logConsole("[GazeboLidarReader] Stopping...")
        self.stop_event.set()
        ...
        self.sensor_logger_instance.logConsole("[GazeboLidarReader] All processes stopped.")
    ...
    def start_live_plot(self):
        if self._plot_process is not None and self._plot_process.is_alive():
            self.sensor_logger.info("[GazeboLidarReader] Plot process already running.")
            return
        ...
        self._plot_process.start()
        self.sensor_logger.info("[GazeboLidarReader] Live plot process started.")
