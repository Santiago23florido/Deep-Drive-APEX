...
```bash
./APEXsim/tools/sim/apex_sim_up.sh --scenario baseline --rviz --slam
```
...
```text
APEXsim/rviz/apex_recognition_live.rviz
```

Con `--slam`, la RViz por defecto pasa a:

```text
APEXsim/rviz/apex_recognition_slam_live.rviz
```
...
ros2 launch rc_sim_description apex_sim.launch.py \
  scenario:=baseline \
  rviz:=true \
  rviz_config:=/home/santiago/AiAtonomousRc/APEXsim/rviz/apex_slam_auto.rviz
```

Armar `recognition_tour` cuando la simulación ya está levantada:

```bash
./APEXsim/tools/sim/apex_arm_recognition_tour.sh
```

Captura completa de una corrida simulada:

```bash
./APEXsim/tools/sim/apex_recognition_tour_sim_capture.sh --scenario tight_right_saturation --timeout-s 60
```

Comparar una corrida real contra una simulada:

```bash
python3 APEXsim/tools/analysis/compare_recognition_tour_runs.py \
  --real-run APEX/data/apex_recognition_tour/<real_run> \
  --sim-run APEXsim/data/apex_recognition_tour/<sim_run>
```

Mapping manual con Xbox + mapa offline final:

```bash
./APEXsim/tools/sim/apex_manual_mapping_up.sh --scenario precision_fusion --rviz
```

Antes, genera una vez el puente Windows:

```bash
./APEXsim/tools/windows/build_apex_xbox_bridge_sim.sh
```

Luego abre en Windows:

```text
APEXsim/tools/windows/dist/apex_xbox_bridge_sim.exe
```
...
```bash
./APEXsim/tools/sim/apex_manual_mapping_finish.sh
```

Ese flujo deja los artefactos en:

```text
APEXsim/data/apex_forward_raw/manual_xbox_<timestamp>/
```
...
cd /home/santiago/AiAtonomousRc/APEXsim/ros2_ws && colcon build --symlink-install --packages-select rc_sim_description apex_telemetry voiture_system
source /home/santiago/AiAtonomousRc/APEXsim/ros2_ws/install/setup.bash
export APEX_SIM_ROOT=/home/santiago/AiAtonomousRc/APEXsim
...
- Usa `control_mode:=manual_windows_bridge` cuando el mando entra por `APEXsim/tools/windows/dist/apex_xbox_bridge_sim.exe` y llega a WSL por el puente de Windows.
