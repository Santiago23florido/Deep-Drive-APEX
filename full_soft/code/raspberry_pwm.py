...
class PWM:
    def __init__(self, channel: int, frequency: float) -> None:
        ...
        self.logger = get_logger("PWM")
        
        self.simulation = platform.machine() not in ("armv7l", "aarch64")
        
        if self.simulation:
            self.logger.warning("PWM running in SIMULATION mode (no /sys/class/pwm found)")
        else:   
            self.logger.info("PWM running in REAL mode")
        ...
        if not self.simulation:
            ...
            self.chip_path = "SIM_PWM"
            self.pwm_dir = f"SIM_PWM{channel}"
    ...
    def _ensure_pwm_channel(self) -> None:
        ...
        The kernel returns EINVAL (errno 22) when the channel is already exported
        (e.g. from a previous run that didn't call unexport on shutdown).
        We treat that specific error as a success — the channel is already there.
        """
        if not os.path.isdir(self.pwm_dir):
            try:
                ...
                self.simple_logger.error(f"Failed to export PWM channel: {e}")
    ...
    def echo(self, message: int, filename: str) -> None:
        if self.simulation:
            self.logger.debug(f"[SIM PWM] {filename} <- {message}")
            return
