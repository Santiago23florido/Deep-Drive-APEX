...
RUN_ROOT="${APEX_RECOGNITION_TOUR_ROOT:-${REPO_ROOT}/APEX/apex_recognition_tour}"
SCENARIO="baseline"
TIMEOUT_S="60"
...
python3 "${REPO_ROOT}/APEX/ros2_ws/scripts/capture/record_recognition_tour.py" \
...
python3 "${REPO_ROOT}/APEX/tools/analysis/plot_recognition_tour_run.py" \
  --run-dir "${RUN_DIR}" \
  >> "${RUN_DIR}/recognition_tour_record.log" 2>&1 || true

if [[ -n "${REAL_RUN}" ]]; then
  python3 "${REPO_ROOT}/APEX/tools/analysis/compare_recognition_tour_runs.py" \
    --real-run "${REAL_RUN}" \
    --sim-run "${RUN_DIR}" \
    >> "${RUN_DIR}/recognition_tour_record.log" 2>&1 || true
fi
