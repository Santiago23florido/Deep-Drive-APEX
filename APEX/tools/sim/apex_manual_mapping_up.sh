...
RUNTIME_DIR="${APEX_RUNTIME_DIR:-${APEX_ROOT}/.apex_runtime/manual_mapping}"
...
EVALUATION_WORLD="${REPO_ROOT}/src/rc_sim_description/worlds/basic_track.world"
...
RUN_DIR="${APEX_ROOT}/apex_forward_raw/${RUN_ID}"
mkdir -p "${RUN_DIR}"
...
if [[ "${SKIP_BUILD}" != "1" ]]; then
  ...
  colcon build \
    --symlink-install \
    --base-paths src APEX/ros2_ws/src \
...
WINDOWS_EXE_PATH="${APEX_ROOT}/tools/windows/dist/apex_xbox_bridge.exe"
...
if [[ ! -f "${WINDOWS_EXE_PATH}" ]]; then
  ...
  echo "  ./APEX/tools/windows/build_apex_xbox_bridge.sh"
fi
if [[ -n "${WSL_DISTRO_NAME:-}" ]]; then
  echo "[APEX] Ruta Windows equivalente:"
  echo "  \\\\wsl$\\${WSL_DISTRO_NAME}\\home\\santiago\\AiAtonomousRc\\APEX\\tools\\windows\\dist\\apex_xbox_bridge.exe"
fi
...
(
  ...
  python3 "${APEX_ROOT}/ros2_ws/scripts/capture/wait_raw_pipeline_ready.py" \
  ...
  python3 "${APEX_ROOT}/ros2_ws/scripts/capture/record_manual_sensorfusion_capture.py" \
    --imu-topic /apex/imu/data_raw \
    --scan-topic /lidar/scan_localization \
...
if [[ "${GENERAL_MAPPER}" == "1" ]]; then
  nohup "${APEX_ROOT}/tools/sim/apex_general_map_publisher_up.sh" \
  ...
  python3 "${APEX_ROOT}/tools/sim/apex_general_mapping_preview_worker.py" \
    --run-dir "${RUN_DIR}" \
    --mapper-script "${REPO_ROOT}/src/rc_sim_description/scripts/apex_general_track_mapper.py" \
...
launch_cmd=(
  "${APEX_ROOT}/tools/sim/apex_sim_up.sh"
  --scenario "${SCENARIO}"
