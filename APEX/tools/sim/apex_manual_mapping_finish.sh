...
python3 -u "${REPO_ROOT}/src/rc_sim_description/scripts/apex_general_track_mapper.py" \
  --run-dir "${RUN_DIR}" \
...
if [[ "${OPEN_RVIZ}" == "1" ]]; then
  nohup bash -lc "
    ...
    source '${REPO_ROOT}/install/setup.bash'
    set -u
    exec rviz2 -d '${APEX_ROOT}/rviz/apex_manual_mapping_offline.rviz'
...
echo "  ./APEX/tools/sim/apex_general_map_publisher_up.sh ${FIXED_MAP_DIR}/fixed_map.yaml ${FIXED_MAP_DIR}/mapping_summary.json"
echo "  rviz2 -d ${APEX_ROOT}/rviz/apex_manual_mapping_offline.rviz"
echo "[APEX] Fixed-map run command:"
echo "  ./APEX/tools/sim/apex_sim_up.sh --scenario baseline --rviz --fixed-map-run ${RUN_DIR}"
