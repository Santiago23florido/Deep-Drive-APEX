...
TRAJECTORY_SUPERVISOR_STATUS_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_STATUS_TOPIC:-/apex/planning/trajectory_supervisor/status}"
...
export APEX_ENABLE_TRAJECTORY_SUPERVISOR="${APEX_ENABLE_TRAJECTORY_SUPERVISOR:-0}"
...
if [[ "${APEX_ENABLE_TRAJECTORY_SUPERVISOR}" == "1" ]]; then
  require_topic "${TRAJECTORY_SUPERVISOR_STATUS_TOPIC}" || {
    postcheck_fail "Missing ${TRAJECTORY_SUPERVISOR_STATUS_TOPIC} in trajectory-supervisor mode"
  }
fi
