...
ENABLE_TRAJECTORY_SUPERVISOR="${APEX_ENABLE_TRAJECTORY_SUPERVISOR:-0}"
...
TRAJECTORY_SUPERVISOR_GLOBAL_PATH_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_GLOBAL_PATH_TOPIC:-}"
TRAJECTORY_SUPERVISOR_GLOBAL_STATUS_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_GLOBAL_STATUS_TOPIC:-}"
TRAJECTORY_SUPERVISOR_SCAN_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_SCAN_TOPIC:-}"
TRAJECTORY_SUPERVISOR_ODOM_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_ODOM_TOPIC:-}"
TRAJECTORY_SUPERVISOR_LOCAL_PATH_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_LOCAL_PATH_TOPIC:-}"
TRAJECTORY_SUPERVISOR_STATUS_TOPIC="${APEX_TRAJECTORY_SUPERVISOR_STATUS_TOPIC:-}"
...
if [[ "${ENABLE_TRAJECTORY_SUPERVISOR}" == "1" ]]; then
  TRAJECTORY_SUPERVISOR_GLOBAL_PATH_TOPIC="${TRAJECTORY_SUPERVISOR_GLOBAL_PATH_TOPIC:-${APEX_FIXED_MAP_ROUTE_PATH_TOPIC:-/apex/planning/fixed_map_path}}"
  TRAJECTORY_SUPERVISOR_GLOBAL_STATUS_TOPIC="${TRAJECTORY_SUPERVISOR_GLOBAL_STATUS_TOPIC:-${APEX_FIXED_MAP_ROUTE_STATUS_TOPIC:-/apex/planning/fixed_map_status}}"
  TRAJECTORY_SUPERVISOR_SCAN_TOPIC="${TRAJECTORY_SUPERVISOR_SCAN_TOPIC:-/lidar/scan_localization}"
  TRAJECTORY_SUPERVISOR_ODOM_TOPIC="${TRAJECTORY_SUPERVISOR_ODOM_TOPIC:-${FUSION_ODOM_TOPIC:-/apex/odometry/imu_lidar_fused}}"
  TRAJECTORY_SUPERVISOR_LOCAL_PATH_TOPIC="${TRAJECTORY_SUPERVISOR_LOCAL_PATH_TOPIC:-/apex/planning/trajectory_supervisor/local_path}"
  TRAJECTORY_SUPERVISOR_STATUS_TOPIC="${TRAJECTORY_SUPERVISOR_STATUS_TOPIC:-/apex/planning/trajectory_supervisor/status}"
  RECOGNITION_TRACKER_PATH_TOPIC="${RECOGNITION_TRACKER_PATH_TOPIC:-${TRAJECTORY_SUPERVISOR_LOCAL_PATH_TOPIC}}"
  RECOGNITION_TRACKER_PLANNING_STATUS_TOPIC="${RECOGNITION_TRACKER_PLANNING_STATUS_TOPIC:-${TRAJECTORY_SUPERVISOR_STATUS_TOPIC}}"
fi
...
if [[ "${ENABLE_TRAJECTORY_SUPERVISOR}" == "1" ]]; then
  python3 -m apex_telemetry.perception.trajectory_supervisor_node \
    --ros-args \
    --params-file "${PARAMS_FILE}" \
    -p "global_path_topic:=${TRAJECTORY_SUPERVISOR_GLOBAL_PATH_TOPIC}" \
    -p "global_status_topic:=${TRAJECTORY_SUPERVISOR_GLOBAL_STATUS_TOPIC}" \
    -p "scan_topic:=${TRAJECTORY_SUPERVISOR_SCAN_TOPIC}" \
    -p "odom_topic:=${TRAJECTORY_SUPERVISOR_ODOM_TOPIC}" \
    -p "local_path_topic:=${TRAJECTORY_SUPERVISOR_LOCAL_PATH_TOPIC}" \
    -p "status_topic:=${TRAJECTORY_SUPERVISOR_STATUS_TOPIC}" &
  track_pid "trajectory supervisor" "$!"
  startup_stage_sleep "trajectory supervisor"
else
  echo "[APEX] Trajectory supervisor disabled for this run"
fi
...
if [[ "${ENABLE_TRAJECTORY_SUPERVISOR}" == "1" ]]; then
  PIPELINE_FEATURES+=("trajectory supervisor")
fi
if [[ "${ENABLE_RECOGNITION_TOUR_TRACKER}" == "1" ]]; then
