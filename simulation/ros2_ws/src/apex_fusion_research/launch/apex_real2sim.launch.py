"""Real2sim closed-loop run: learned odometry + SLAM driving a validation format.
    ...
    with Gazebo noise)
      ...
      -> apex_track_driver: pure pursuit + speed plan of the format on
           T_world_map * map->odom * odom pose  (world = nominal start pose)
...
does (reference path, speed plan, driver gains, actuator perturbation, start
pose with its seeded placement errors, true sensor mounts drawn for that
trajectory). The car only knows its nominal calibration: base_link -> laser
and -> imu_link are the nominal mounts, and the start line is the nominal
start pose.
...
def _setup(context, *_args, **_kwargs):  # noqa: ANN001, ANN202
    ...
    nx, ny, nyaw = rs["spawn"] if start_pose == "true" else rs["nominal_start"]
    ...
    (run_dir / "run_config.json").write_text(json.dumps({
        ...
        "drive": drive, "start_pose": start_pose, "spawn_true": rs["spawn"], "nominal_start": rs["nominal_start"], "trajectory_key": tkey,
    ...
    stack = [
        ...
        *slam,
        ...
        Node(package=PKG, executable="run_referee_node", name="apex_run_referee", output="screen",
             parameters=[{"track": track, "motion": motion, "seed": seed, "laps": laps, "output_dir": str(run_dir), **common}]),
    ...
    if arg("rviz").lower() in ("1", "true", "yes"):
        stack.append(Node(package="rviz2", executable="rviz2", name="real2sim_rviz", output="log",
                          arguments=["-d", str(sim / "ros2_ws" / "src" / PKG / "rviz" / "real2sim.rviz")], parameters=[common]))
...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("checkpoint", default_value=str(sim / "learning" / "outputs" / "real2sim_v2" / "hybrid_submap" / "best_model.pt")),
        DeclareLaunchArgument("device", default_value="cpu", description="cpu (~20 ms per scan) | cuda"),
        DeclareLaunchArgument("drive", default_value="estimate", description="estimate (closed loop) | truth (diagnostic)"),
